...
class Encryption{
    ...
    public static function decryNameId($slug, $table){
            if(empty($slug)){
                throw new Exception("the id to decrypt can't be empty");
            }
         ...
         $database->bindValue(':slug', (string)$slug);
         $database->execute();

         $meta = $database->fetchAssociative();

         return urldecode($meta["id"]);
    }

    public static function encryNameId($id, $table){
         $database = Database::openConnection();
         $query  = "SELECT slug ";
         $query .= "FROM ".$table. " ";
         $query .= "WHERE id = :id ";
